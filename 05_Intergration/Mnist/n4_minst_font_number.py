...
# 폰트 설정하기
# ttf_list = glob.glob("C:\Windows\Fonts\*.ttf")    # Mac
# ttf_list += glob.glob("~/Library/Fonts/*.ttf")    # Mac
# tf_list += glob.glob("/usr/share/fonts/*.ttf)     # Ubuntu
# tf_list += glob.glob("~/.fonts/*.ttf)             # Ubuntu
...
# 상위폴더도 없을 때 이 명령어를 쓰면 상위 폴더도 만듬

# 회전하거나 확대해서 데이터 양 늘리기
...
for path in ttf_list:
    ...
    for cnt in contours:
        ...
        num = ima[y:y+h, x:x+w] # 부분 이밎 ㅣ추출하기
        ww = w if w > h else h
        wx = (ww - w) // 2
        wy = (ww - h) // 2
        spc = np.zeros((ww,ww))
